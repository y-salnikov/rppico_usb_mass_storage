...
uint __not_in_flash_func( storage_get_flash_capacity )(void)
{
    ...
    uint32_t fsize=(1 << rxbuf[3]);
	uintptr_t flash_end_address = (uintptr_t)&__flash_binary_end;
	// Calculate the start of the next 4KiB aligned sector
	uintptr_t nearest_block = (flash_end_address + FLASH_SECTOR_SIZE) & 0xfffff000;

	// Convert the absolute address to a flash offset
	uint32_t flash_offset = nearest_block - XIP_BASE;
    FS_BASE=(void*)flash_offset;
    image_file_size=(fsize-flash_offset)-8192;
    return fsize-flash_offset;
}
...
//    printf("Block #%d @ 0x%0X : %d to read\n",block,FS_BASE + (block * pico_cfg.block_size) + off,pico_cfg.block_size);
...
//    printf("Block #%d @ 0x%0X : %d to erase\n",block,p,pico_cfg.block_size);
    uint32_t ints = save_and_disable_interrupts();
...
#endif
	uint32_t	actual_size=storage_get_flash_capacity();
	pico_cfg.block_count=actual_size / FLASH_SECTOR_SIZE;

    if (format)
        lfs_format(&pico_cfg);
    // mount the filesystem
    int err=lfs_mount(&pico_cfg);
    if(err)
    {
		lfs_format(&pico_cfg);
